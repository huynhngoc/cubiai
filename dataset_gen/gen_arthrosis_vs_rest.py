...
h5_filename = '//nmbu.no/Research/Project/CubiAI/preprocess/datasets/800_arthrosis_vs_rest.h5'
rs=27
...
df = ds[ds['diagnosis']==6].sample(n=8, random_state=rs, axis = 0, replace=False) # 25% of OCD is only 4, which is too little
for d in range(7):
    if d+1 == 6: 
        print(d+1, sum(df['diagnosis']==d+1))
        continue
    elif d+1 == 1 or d+1 == 2 or d+1 == 4:
        new = ds[ds['diagnosis']==d+1].sample(frac=0.2, random_state=rs, axis = 0, replace=False)
        df = pd.concat([df, new], ignore_index=True)
        print(d+1, sum(df['diagnosis']==d+1)) # print number of samples in each class
    else:
        new = ds[ds['diagnosis']==d+1].sample(frac=0.6, random_state=rs, axis = 0, replace=False)
        df = pd.concat([df, new], ignore_index=True)
        print(d+1, sum(df['diagnosis']==d+1)) # print number of samples in each class
...
diagnosis[diagnosis == 2] = 0
diagnosis[diagnosis == 3] = 1
diagnosis[diagnosis == 4] = 0
