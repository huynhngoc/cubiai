...
h5_filename = '//nmbu.no/LargeFile/Project/CubiAI/preprocess/datasets/800_normal_abnormal_1.h5'
...
df = df.sample(frac=0.25, random_state=124, axis = 0, replace=False)
...
for i,d in enumerate(diagnoses): # Change all diagnoses to numbers from 1 through 7 (Not normal samples)
    print(i+1, d,sum(df['diagnosis_raw']==d)) # print number of samples in each class
    df.diagnosis.iloc[np.where(df['diagnosis_raw']==d)] = i+1

np.unique(df['diagnosis']) # Want all diagnoses from 0 through 7
